    ...
    <v-dialog v-model="internalDialog">
        <v-card>
            ...
            <v-container style="width: 50%;">
                <v-alert v-if="this.aList.length == 0" class="d-flex justify-center" title="아직은 새로운 알림이 없네요!"
                    type="success" style="opacity: 0.8;"></v-alert>

                <v-alert color="#2A3B4D" density="compact" icon="mdi-chat" theme="dark"
                    class="d-flex justify-center chat-alert" @click="this.$router.push('/member/chatting-list');"
                    v-if="this.localChatCnt >= 1">
                    <p> {{ this.localChatCnt }}개의 새로운 채팅이 있어요! </p>
                </v-alert>

                <div v-for="al in this.aList" :key="al.id">
                    <v-alert border="top" type="warning" variant="outlined" class="d-flex justify-center"
                        v-if="al.isRead == 'N'">
                        <p> {{ al.title }} </p>
                        <p> {{ al.content }} </p>
                        <p style="color: gray; font-size: small"> {{ getDay(al.createdAt) }} {{ getTime(al.createdAt) }}
                        </p>
                    </v-alert>


                    <v-alert style="margin-top: 10px;" color="#2A3B4D" density="compact" icon="mdi-firework" type="info"
                        variant="tonal" class="d-flex justify-center" v-if="al.isRead == 'Y'">
...
export default {
    ...
    async created() {
        ...
        this.internalDialog = this.showDialog;
    },
    methods: {
        exist() {
